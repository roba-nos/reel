// --- Asynchronous Asset Loading (Prevents freezing when offline) ---
...
function initSupabase() {
    ...
    supabaseLoadPromise = new Promise((resolve) => {
        ...
        if (window.supabase) {
            ...
            // Load script locally from assets to ensure offline support
            const script = document.createElement('script');
            script.src = 'assets/supabase.js';
            script.async = true;
            script.onload = createClient;
            script.onerror = () => {
                console.warn('Local Supabase SDK failed to load. Trying CDN fallback...');
                const cdnScript = document.createElement('script');
                cdnScript.src = 'https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2';
                cdnScript.onload = createClient;
...
// Inject Google Fonts asynchronously to prevent render blocking
(function injectFonts() {
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = 'https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=Noto+Sans+Arabic:wght@300;400;500;600;700&display=swap';
    link.media = 'print';
...
window.showToast = (msg, icon = 'fa-circle-check') => {
    ...
    // Use class-based animation to avoid overwriting CSS transforms
...
// --- Global Promotion/New Addition Notification System ---
(function initGlobalPromotions() {
    ...
    style.innerHTML = `
        ...
        .global-promo-notif.active { transform: translateX(0); }
        .promo-thumb { width: 55px; height: 75px; border-radius: 8px; object-fit: cover; flex-shrink: 0; box-shadow: 0 5px 15px rgba(0,0,0,0.3); }
        .promo-info { flex: 1; display: flex; flex-direction: column; justify-content: center; min-width: 0; }
        .promo-tag { color: #fbbf24; font-size: 10px; font-weight: 900; text-transform: uppercase; margin-bottom: 2px; display: flex; align-items: center; gap: 4px; }
        .promo-name { color: white; font-size: 13px; font-weight: 700; margin-bottom: 8px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .promo-go { background: #e50914; color: white; padding: 5px 14px; border-radius: 8px; font-size: 11px; font-weight: 800; width: fit-content; text-decoration: none !important; transition: 0.2s; }
        .promo-go:hover { transform: scale(1.05); filter: brightness(1.1); }
        .promo-bar { position: absolute; bottom: 0; left: 0; height: 3px; background: #e50914; width: 100%; transform-origin: left; }
        @keyframes promoProgress { from { transform: scaleX(1); } to { transform: scaleX(0); } }
        @media (max-width: 480px) {
            .global-promo-notif { bottom: auto; top: 20px; right: 15px; left: 15px; width: auto; transform: translateY(-150%); }
            .global-promo-notif.active { transform: translateY(0); }
        }
    `;
    ...
    window.addEventListener('DOMContentLoaded', async () => {
        ...
        const stats = JSON.parse(localStorage.getItem('reelar_notif_stats') || JSON.stringify({ date: today, count: 0, last_seen_id: 0 }));
        ...
        try {
            ...
            const sorted = series.sort((a,b) => (b.id || 0) - (a.id || 0));
            const latest = sorted[0];
            const isTrulyNew = latest.id > stats.last_seen_id;

            // Decision Logic: 
            // 1. If it's a New Addition: Show it (until last_seen_id is updated)
            // 2. If it's a Suggestion:
            //    - Only show if daily limit (< 5) not reached
            //    - AND only show ONCE per browser session to avoid annoyance
            const sessionShown = sessionStorage.getItem('reelar_suggestion_session_shown');
            
            if (!isTrulyNew) {
                if (stats.count >= 5 || sessionShown) return;
            }

            // Create Element
            const div = document.createElement('div');
            div.className = 'global-promo-notif';
            div.innerHTML = `
                <img src="${latest.poster || latest.poster_url || 'assets/no-poster.png'}" class="promo-thumb">
                <div class="promo-info">
                    <span class="promo-tag">${isTrulyNew ? 'إضافة جديدة ✨' : 'نقترح لك 🔥'}</span>
                    <div class="promo-name">${latest.title}</div>
                    <a href="series.html?id=${latest.id}" class="promo-go">شاهد الآن</a>
                </div>
                <div class="promo-bar"></div>
            `;
            document.body.appendChild(div);

            // Show after longer delay (10s) to be less intrusive
            setTimeout(() => {
                div.classList.add('active');
                div.querySelector('.promo-bar').style.animation = 'promoProgress 8s linear forwards';
                
                // Update Stats
                if (isTrulyNew) {
                    stats.last_seen_id = latest.id;
                } else {
                    stats.count++;
                    sessionStorage.setItem('reelar_suggestion_session_shown', 'true');
                }
                localStorage.setItem('reelar_notif_stats', JSON.stringify(stats));
...
window.addEventListener('offline', () => {
    showToast('أنت الآن غير متصل بالإنترنت. يرجى التحقق من الشبكة لمتابعة المشاهدة.', 'fa-wifi');
    document.body.classList.add('is-offline');
});
window.addEventListener('online', () => {
    showToast('تمت استعادة الاتصال بالإنترنت بنجاح!', 'fa-wifi');
    document.body.classList.remove('is-offline');
});
if (!navigator.onLine) {
    setTimeout(() => showToast('لا يوجد اتصال بالإنترنت حالياً. تعمل الواجهة من الذاكرة المؤقتة.', 'fa-wifi'), 1000);
}
...
const db = {
    ...
    async getConfig(forceRefresh = false) {
        ...
        const fetchTask = (async () => {
            try {
                await initSupabase();
                if (!supabaseClient) return cached || getLocalAppConfig();
    ...
    async getSeriesByTitle(title) {
        await initSupabase();
        const { data } = await supabaseClient.from('series').select('*').eq('title', title).maybeSingle();
        return data;
    },
    ...
    async ensureCategoriesExist(categoryString) {
        await initSupabase();
        if (!categoryString) return;
        const cats = categoryString.split(',').map(c => c.trim()).filter(c => c);
        
        // Use cached categories to avoid excessive DB calls, but force refresh if we suspect missing ones
        const existing = await this.getCategories(); 
        const existingNames = existing.map(c => (typeof c === 'string' ? c : c.name).trim());
        
        for (const catName of cats) {
            if (!existingNames.includes(catName)) {
                console.log(`Auto-adding missing category: ${catName}`);
                await this.saveCategory(catName);
            }
        }
        // Refresh cache after additions
        await this.getCategories(true);
    },
    async saveSeries(s) {
        await initSupabase();
        // 1. Detect columns if not cached
        if (!this._seriesColumns) {
            try {
                const { data } = await supabaseClient.from('series').select('*').limit(1);
                if (data && data.length > 0) this._seriesColumns = Object.keys(data[0]);
            } catch (e) { console.warn("Failed to detect columns:", e); }
        }
        const cols = this._seriesColumns || [];
        console.log('--- Series Table Columns Detected ---', cols);
        if (cols.length > 0 && !cols.includes('category')) {
            console.warn('تنبيه: عمود "category" (نصي) مفقود في قاعدة البيانات. لهذا السبب يتم حفظ تصنيف واحد فقط في "category_name" (الذي يقبل تصنيفاً واحداً فقط للربط). يرجى إضافة عمود باسم "category" من نوع text في جدول series.');
        }

        // 2. Auto-ensure categories exist (individually)
        await this.ensureCategoriesExist(s.category);

        // 3. Prepare category data for FK vs Text columns
        const categoryParts = (s.category || '').split(',').map(c => c.trim()).filter(c => c);
        const firstCategoryInput = categoryParts[0] || 'General';
        
        // Find exact name from DB to satisfy FK (case-insensitive matching)
        const existingCats = await this.getCategories();
        const matchedCat = existingCats.find(c => {
            const name = typeof c === 'string' ? c : c.name;
            return name.trim().toLowerCase() === firstCategoryInput.toLowerCase();
        });
        const firstCategory = matchedCat ? (typeof matchedCat === 'string' ? matchedCat : matchedCat.name) : firstCategoryInput;

        // 4. Build payload based on actual columns
        // We initially try to put the FULL string in both, let the FK retry logic handle it
        const payload = {};
        const mapping = {
            title: s.title,
            description: s.description,
            type: s.type || 'series',
            category_name: s.category, // Try full string first
            category: s.category,      // Full string
            poster_url: s.poster,
            poster: s.poster,
            trailer_url: s.video,
            video: s.video,
        ...
        // Special case fallback if detection failed
        if (cols.length === 0) {
            payload.title = s.title;
            payload.description = s.description;
            payload.type = s.type || 'series';
            payload.category_name = s.category;
            payload.poster_url = s.poster;
        }

        const performSave = async (currentPayload) => {
            if (s.id) {
                const { error } = await supabaseClient.from('series').update(currentPayload).eq('id', s.id);
                if (error) throw error;
                return s.id;
            } else {
                const { data, error } = await supabaseClient.from('series').insert([currentPayload]).select().single();
                if (error) throw error;
                return data.id;
            }
        };
        ...
        try {
            try {
                seriesId = await performSave(payload);
            } catch (error) {
                // If FK violation, retry with only the first category for category_name
                if (error.message && error.message.includes('foreign key') && payload.category_name !== firstCategory) {
                    console.warn("FK violation detected, retrying with first category only for category_name...");
                    payload.category_name = firstCategory;
                    seriesId = await performSave(payload);
                } else {
                    throw error;
                }
            ...
            // 5. Sync Episodes & Linked Parts
            if (seriesId && (s.episodes || s.linked_parts)) {
                await initSupabase();
                ...
                if (finalEpisodes.length > 0) {
                    ...
                    if (epErr) {
                        // Fallback: Try without season column
            ...
            console.error('Save Series Failed:', e.message || e);
            return false;
        }
    },
    ...
    async saveConfig(config) {
        await initSupabase();
        const { error } = await supabaseClient.from('site_config').upsert({ id: 1, config_data: config });
    ...
    async saveCategory(name, oldName = null) {
        ...
        try {
            if (oldName) {
                ...
                // Check for existence one last time to be safe
                const { data } = await supabaseClient.from('categories').select('name').eq('name', name).maybeSingle();
                if (data) return true;

                const { error } = await supabaseClient.from('categories').insert([{ name }]);
                if (error) {
                    if (error.code === '23505') return true; // Unique violation is fine
                    throw error;
                }
                return true;
            }
        } catch (e) {
            console.error('Save Category Failed:', e.message || e);
            return false;
        }
    },
