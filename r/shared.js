...
const db = {
    ...
    async getSeries(forceRefresh = false) {
        ...
        const CURRENT_VER = '2.1'; // Increment this to force all clients to refresh
        if (cacheVer !== CURRENT_VER) {
            console.log('[DEBUG] Series linking logic updated. Forcing fresh sync...');
            forceRefresh = true;
        ...
        const fetchTask = (async () => {
            console.log('[DEBUG] Fetching fresh series data...');
            try {
                // REAL SCHEMA: episodes has id, title, video_url. series has id, title, etc.
                ...
                if (!error && data) {
                    ...
                    // Transitive Link Resolution (Group-based)
                    // We iterate multiple times to ensure all indirect links are captured
                    let changed = true;
                    let iterations = 0;
                    while (changed && iterations < 5) {
                        ...
                        iterations++;
                        mapped.forEach(s => {
                            const oldSize = s.linked_ids.length;
                            // Find everyone who links to me or I link to them
                            mapped.forEach(other => {
                                if (s.id === other.id) return;
                                
                                // If I link to them, or they link to me, we share our link pools
                                const connected = s.linked_ids.includes(other.id) || other.linked_ids.includes(s.id);
                                if (connected) {
                                    // Add their IDs to my pool
                                    ...
                                    // Add myself to their pool if not there
                    ...
                    console.log(`[DEBUG] Transitive Sync Complete. Groups resolved in ${iterations} iterations.`);
    ...
    async getSeriesById(id) {
        try {
            // Use the centralized logic from getSeries which handles bi-directional linking
            const allSeries = await this.getSeries();
            const found = allSeries.find(x => String(x.id) === String(id));
            
            if (found) {
                // Fetch fresh episodes for the player
                const { data: episodes, error } = await supabaseClient.from('episodes').select('*').eq('series_id', id).order('order_index', { ascending: true });
                return {
                    ...found,
    ...
    async getConfig(forceRefresh = false) {
        ...
        const fetchTask = (async () => {
            try {
                ...
                // If 406 or empty, we return local defaults
            } catch (e) {}
