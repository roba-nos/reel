...
const db = {
    ...
    async getCategories(forceRefresh = false) {
        ...
        const fetchTask = (async () => {
            try {
                ...
                if (!error && data) {
                    const mapped = data.map(c => c.name || 'بدون اسم');
        ...
        return (!forceRefresh && cached && cached.length > 0) ? cached : await fetchTask;
    },

    // 2. Series & Episodes
    async getSeries(forceRefresh = false) {
        const cached = JSON.parse(localStorage.getItem('reelar_series_cache'));
        const fetchTask = (async () => {
            try {
                ...
                if (!error && data) {
                    // Add episodes_count: episodes table count + 1 if trailer/ep1 exists on series
                    const mapped = data.map(s => ({
                        ...s,
                        episodes_count: (s.episodes || []).length + ((s.trailer_url || s.video) ? 1 : 0),
                        episodes: undefined // Don't cache full episodes array
                    }));
    ...
    async getSeriesById(id) {
        const { data, error } = await supabaseClient.from('series').select('*, episodes(*)').eq('id', id).single();
        if (error || !data) return null;
        return {
            ...data,
            episodes: (data.episodes || []).sort((a, b) => (a.order_index || 0) - (b.order_index || 0)).map(e => ({ 
                ...e,
    ...
    async getAds(forceRefresh = false) {
            ...
            try {
                ...
                if (data) {
                    const mapped = data.map(a => ({ 
                        ...
                        id: a.id, 
                        active: a.is_active,
                        // If columns don't exist, they'll be in ad_data (already spread above)
                        // but we ensure they have at least 0
                        views: a.views_count || a.ad_data.views || 0,
                        clicks: a.clicks_count || a.ad_data.clicks || 0
                    }));
        ...
        // For public use, we usually only want ACTIVE ads
        return allAds.filter(a => a.active);
    },
    ...
    async getConfig(forceRefresh = false) {
        ...
        const fetchTask = (async () => {
            try {
                ...
                if (!error && data) {
                    localStorage.setItem('reelar_config_cache', JSON.stringify(data.config_data));
                    return data.config_data;
                }
            } catch (e) {}
            return cached || getLocalAppConfig();
        })();
        return (!forceRefresh && cached) ? cached : await fetchTask;
    },
    ...
    async saveSeries(s) {
        ...
        try {
            let seriesId = s.id;
            if (seriesId) {
                await supabaseClient.from('series').update(payload).eq('id', seriesId);
            } else {
            ...
            if (s.episodes) {
                ...
                const epPayloads = s.episodes.map((ep, idx) => ({
                    series_id: seriesId,
                    title: ep.title,
                    video_url: ep.video,
                    order_index: idx + 2 // Matching our new numbering logic
                }));
                if (epPayloads.length > 0) {
                    await supabaseClient.from('episodes').insert(epPayloads);
                }
            }
    ...
    async saveAd(a) {
        const payload = {
            ...
            ad_data: { ...a.ad_data, name: a.name, type: a.type }
        };
        try {
            if (a.id) {
                const { error } = await supabaseClient.from('ads').update(payload).eq('id', a.id);
                if (error) { showToast("خطأ في التحديث: " + error.message, 'error'); return false; }
                localStorage.removeItem('reelar_ads_cache');
                return true;
            } else {
                const { error } = await supabaseClient.from('ads').insert([payload]);
                if (error) { showToast("خطأ في الإضافة: " + error.message, 'error'); return false; }
                localStorage.removeItem('reelar_ads_cache');
                return true;
            }
        } catch (e) { showToast("خطأ تقني: " + e.message, 'error'); return false; }
    },
    async deleteAd(id) {
...
async function isFavorite(id) {
    const profile = await getUserProfile();
    return profile ? (profile.favorites || []).includes(String(id)) : false;
}
...
async function getAds(forceRefresh = false) { return await db.getAds(forceRefresh); }
            ...
            <div class="card-img-wrapper">
                ...
                <div class="card-overlay">
                    <div class="card-actions">
                        <button class="card-action-btn ${isFav ? 'active' : ''}" onclick="event.stopPropagation(); toggleFavorite('${series.id}', this)" title="${t['save']}">
                            <i class="fa-solid fa-bookmark" style="${isFav ? 'color: var(--accent);' : ''}"></i>
                        </button>
                        <button class="card-action-btn" onclick="event.stopPropagation();" title="${t['like']}">
                            <i class="fa-solid fa-heart"></i>
                        </button>
                        <button class="card-action-btn" onclick="event.stopPropagation(); shareSeries('${series.id}')" title="${t['share']}">
